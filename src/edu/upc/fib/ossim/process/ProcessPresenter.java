...
//->
		if ("verrou".equals(spin.getName())) { 
			// lock update number
			context.setVerrouNumber((int)spin.getValue());
		}
	}
...
//->
		String[] indique = {"",""};
		if(((FormProcess) form).getTable().getRowCount() > 5){
...
//->	isoler l'action après événement sur la colonne I/O
...
//->	actions sur les colonnes de la variable conditionnelle
						} else if (burstsTable.isCellSelected(r, 3)) {
							burstsTable.setValueAt(new ColorCell(indique[0], Color.LIGHT_GRAY), r, 3);
							burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 4);
						} else if (burstsTable.isCellSelected(r, 4)) {
							burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 3);
							burstsTable.setValueAt(new ColorCell(indique[1], Color.LIGHT_GRAY), r, 4);
//-> 	actions sur les colonnes de ressources
						//action sur la colonne P
						} else if (burstsTable.isCellSelected(r, 5)) {
							if(((ColorCell) burstsTable.getValueAt(r, 5)).getColor().equals(Color.LIGHT_GRAY)){
								burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 5);		
								burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 6);
							}else {
								burstsTable.setValueAt(new ColorCell("P", Color.LIGHT_GRAY), r, 5);		
								burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 6);
							}
							
						//action sur la colonne V
						} else if (burstsTable.isCellSelected(r, 6)) {
							burstsTable.setValueAt(new ColorCell("", Color.WHITE), r, 5);
							burstsTable.setValueAt(new ColorCell("V", Color.LIGHT_GRAY), r, 6);
